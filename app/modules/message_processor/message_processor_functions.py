...
async def process_message_async(user_id: str, text: str, conversation_manager, ai_service: dict, whatsapp_service: dict, system_prompt: str = None, product_radar: dict = None) -> dict:
    ...
    try:
        ...
        logger.info(f"Conversation manager disponível: {conversation_manager is not None}")

        # Processar entrada no conversation manager
        if conversation_manager:
            logger.info("Chamando conversation_manager.process_incoming_message")
            is_first_today, context_messages = await conversation_manager.process_incoming_message(user_id, text)
            logger.info(f"Resultado: is_first_today={is_first_today}, context_messages={len(context_messages) if context_messages else 0}")
            ...
            logger.info(f"Estratégia determinada: {strategy}")

            if strategy == 'immediate':
                # Se for primeira mensagem do dia, enviar mensagem de boas-vindas IMEDIATAMENTE
                if is_first_today:
                    logger.info("Enviando mensagem de boas-vindas do RADAR")
            ...
            logger.warning("Conversation manager não disponível, usando fallback")
